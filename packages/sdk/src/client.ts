...
import { Home, type HomeListResponse } from "./resources/home";
import { AuditLogs, type AuditLogListResponse, type AuditLogCreateResponse, type AuditLogRetrieveResponse, type AuditLogCreateParams } from "./resources/audit-logs";
import { Settings, type SettingListResponse, type SettingCreateResponse, type SettingRetrieveResponse, type SettingPatchResponse, type SettingCreateParams, type SettingPatchParams } from "./resources/settings";
import { Ingredients, type IngredientListResponse, type IngredientCreateResponse, type IngredientRetrieveResponse, type IngredientPatchResponse, type IngredientListAuditLogResponse, type IngredientCreateParams, type IngredientPatchParams } from "./resources/ingredients";
...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env["SCOTT-W-JONES-HOTMAIL-COM-TEAM_BASE_URL"].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env["SCOTT-W-JONES-HOTMAIL-COM-TEAM_LOG"] or 'warn' if it isn't set.
   */
...
export type DeeliciousAPIOptions = ClientOptions;

/**
 * API Client for interfacing with the DeeliciousApi API.
 */
export class DeeliciousAPI {
   ...
   * API Client for interfacing with the DeeliciousApi API.
   *
   * @param {string} [opts.baseURL=process.env["SCOTT-W-JONES-HOTMAIL-COM-TEAM_BASE_URL"] ?? http://localhost:9000] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv("SCOTT-W-JONES-HOTMAIL-COM-TEAM_BASE_URL"),
    ...opts
  }: ClientOptions = {}) {
    ...
    this.timeout = options.timeout ?? DeeliciousAPI.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv("SCOTT-W-JONES-HOTMAIL-COM-TEAM_LOG"), "process.env[\"SCOTT-W-JONES-HOTMAIL-COM-TEAM_LOG\"]", this) ??
      defaultLogLevel;
    ...
    const customHeadersEnv = readEnv("SCOTT-W-JONES-HOTMAIL-COM-TEAM_CUSTOM_HEADERS");
    if (customHeadersEnv) {
  ...
  private async resolveAuthOption(optionName: string, value: string | AuthTokenProvider | null | undefined): Promise<string | undefined> {
    ...
    if (!token) throw new Errors.DeeliciousAPIError(`Expected '${optionName}' to resolve to a non-empty string.`);
    return token;
  }
  ...
  private resolveAuthOptionSync(optionName: string, value: string | AuthTokenProvider | null | undefined): string | undefined {
    ...
    if (typeof token !== "string" || !token) throw new Errors.DeeliciousAPIError(`Expected '${optionName}' to resolve to a non-empty string.`);
    return token;
  }

  static DeeliciousAPI = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static DeeliciousAPIError = Errors.DeeliciousAPIError;
...
DeeliciousAPI.Home = Home;
DeeliciousAPI.AuditLogs = AuditLogs;
DeeliciousAPI.Settings = Settings;
DeeliciousAPI.Ingredients = Ingredients;

export declare namespace DeeliciousAPI {
